# Functional Requirements Specification — FRS v1.0.0
...
| Value at Risk (VaR) confidence | `VAR_CONFIDENCE` | 95 | % (e.g. 95% VaR) |
| Conditional VaR (CVaR) confidence | `CVAR_CONFIDENCE` | 95 | % |
| Max deployment + maintenance cost vs ROI | `COST_ROI_MAX_RATIO` | 5 | % (deploy + maintenance cost ≤ 5% of annual ROI) |
| Max acceptable latency (trade execution path) | `LATENCY_TRADE_MS` | 2000 | ms |
| Withdrawal cap (per single withdrawal) | `WITHDRAWAL_CAP_PCT` | 10 | % of current investable capital (configurable per user profile) |

**Approval:** User to confirm or amend the above table before FRS v1.0.0 is frozen.
...
**Human-friendly:**  
At any time, total capital (invested + cash) must never be less than the initial capital plus the sum of all intermediate bank deposits. The system must never reduce the “protected base” (initial capital + deposits).

*Example:* If you started with 100 and later deposited 50, the system must never let total capital fall below 150.
...
- **VaR and CVaR:** Limits and confidence levels as defined in §3.  
...
- **User approval:** This FRS v1.0.0 is effective only after explicit User approval.
- **Later versions:** Minor changes and optimisations post-production will be documented in subsequent FRS versions (e.g. v1.1.0) and remain the single source of truth.
...
|---|------------|----------------------------|
| 1 | Capital preservation | \( C(t) \geq C_0 + \sum D_i \) for all \( t \). \( C_0 \) = initial capital; \( D_i \) = deposits. |
...
| 5 | Edge-cases | VaR/CVaR at specified confidence; whale/shark rules; hallucination detection; red-team criteria. |
